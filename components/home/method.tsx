      ...
      <ol className="relative grid auto-rows-fr gap-px [&>*]:h-full sm:grid-cols-3">
        ...
        {STEPS.map((step, i) => (
          <Reveal as="li" key={step.n} delay={i * 100}>
            <div className="group relative flex h-full flex-col gap-4 rounded-2xl border border-stroke bg-surface p-6 shadow-[0_2px_20px_-8px_rgba(11,22,63,0.08)] transition-all duration-300 hover:border-primary/30 hover:shadow-[0_8px_32px_-12px_rgba(23,63,234,0.18)]">
              <div className="flex items-center gap-4">
                <span className="relative font-display text-[2.75rem] font-semibold leading-none text-stroke">
                  {step.n}
                  <span
                    aria-hidden
                    className="absolute inset-0 font-display text-[2.75rem] font-semibold leading-none text-primary opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                  >
                    {step.n}
                  </span>
                </span>
